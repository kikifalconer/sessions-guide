...
> **Audit-phase note (2026-07-26):** carried forward from the original project. Phase 0 of `backend-audit-plan.md` diffs this document against the live database and corrects any drift — until then, treat the live DB as authoritative where they disagree. Known suspect: the `bookings.google_event_id` "outbound creation not yet wired" note (see platform-state.md discrepancy #1).
...
**Coherence warning (TD5):** no DB CHECK ties price/price_min/price_max or confirmation_mode to pricing_model. Every write MUST route through `validateSessionTypeInput` (`src/lib/sessionType.ts`).
...
location_lat        numeric(9,6) null  -- for geo queries
location_lng        numeric(9,6) null  -- for geo queries
...
seeker_id             uuid null references auth.users(id)  -- null only on historical guest rows (D20)
guest_name            text  -- historical guest rows only
guest_email           text  -- historical guest rows only
...
seeker_token          text not null unique default encode(gen_random_bytes(32), 'hex')  -- added in 0004; opaque 256-bit per-booking bearer token. Powers the cancel link AND the review link (never mint a second token)
google_event_id       text  -- added in 0008; nullable; Google Calendar event id for outbound sync (delete on cancel). Wiring status = audit discrepancy #1
...
offsite_paid_at       timestamptz  -- added in 0016; guide-reported receipt of an offsite payment, null until reported. Records when the offsite obligation was settled — does NOT affect payment_status, refund math, or hold durability (OD-2). Offsite rows are an obligation between guide and seeker, never a platform payment.
created_at            timestamptz not null default now()
updated_at            timestamptz not null default now()
```
...
```
A non-cancelled `pending_payment` row IS the slot hold during checkout. Abandoned on-platform holds (`payment_status = 'unpaid'`, older than 30 min) are lazily cancelled during slot generation. Offsite `pending_payment` rows (`payment_status = 'offsite'`) are durable and never auto-expired (open decision OD-2).
...
refresh_token   text not null  -- PLAINTEXT (TD1 debt; encrypt-at-rest pre-launch gate, target 2026-09-01)
...
-- Shape matches the slot generator's BookedWindow. Whether these windows are
-- actually merged into public slot suppression = audit GAP-2.
id              uuid primary key default gen_random_uuid()
practitioner_id uuid not null references practitioners(id) on delete cascade
start_datetime  timestamptz not null
end_datetime    timestamptz not null
...
guest_email     text  -- populated for historical guest bookers
guest_name      text
...
updated_at      timestamptz not null default now()
```
Guest-matching logic deprecated for new records (D20).

---

## favorites
```sql
-- added in 0017; a seeker saving a practitioner as a favorite. Same identity
-- pattern as clients.seeker_id — FK straight to auth.users(id), not
-- seekers(id).
...
```
Composite primary key makes save idempotent (upsert, ignore duplicates) and unsave a plain delete — no separate uniqueness constraint needed.

**Index (added in 0017):** `favorites_seeker_id_created_at_idx` on `favorites (seeker_id, created_at desc)` — backs the seeker's own favorites list, newest first.

RLS: enabled with NO policies, per 0010 — anon/authenticated get nothing; all access via the service-role client pattern.
...
practitioner_id uuid not null references practitioners(id)
reviewer_id     uuid null references auth.users(id)  -- null on historical guest submissions
reviewer_name   text not null
rating          int not null check (rating between 1 and 5)
body            text
is_published    boolean not null default false  -- default is a moderation lever; submission path flips true (auto-publish, D8)
is_featured     boolean not null default false  -- practitioner can feature one review (action deferred)
created_at      timestamptz not null default now()
```

**Unique index (added in 0006):** `reviews_one_per_booking` — unique on `booking_id`, one review per booking.

Guest submission at `/review/[token]` reuses `bookings.seeker_token`; authenticated submission shares `src/lib/reviewSubmit.ts`. Consumers: profile rating aggregation (`is_published = true` only) and the submission flow.
...
-- reporter PII. Rate-limit/auth pending (RL-1).
id         uuid primary key default gen_random_uuid()
review_id  uuid not null references reviews(id) on delete cascade
reason     text
created_at timestamptz not null default now()
```
Index: `review_reports_review_id (review_id)`. Write path: public `reportReview` server action (service role) appends a row and sends a Resend notice to `REPORT_NOTICE_EMAIL` (default `hello@sessions.guide`) **only on the first report per review**. Reviews page: `/[slug]/reviews`.
...
updated_at              timestamptz not null default now()
```
Grandfathered/comped practitioners have NO row here; webhook logic must never downgrade a practitioner lacking a subscription row (D24).

**Partial unique index (added in 0014, F-23):** `subscriptions_one_active_per_practitioner` on `subscriptions (practitioner_id)` where `status in ('active', 'trialing', 'past_due')`. Cancelled/unpaid/incomplete rows are unconstrained history and may accumulate freely; only the entitlement-granting statuses are limited to one per practitioner. A webhook upsert that would create a second such row now fails at the DB rather than silently duplicating — Stripe retries the failed write rather than the practitioner ending up billed twice. Applied and verified in prod 2026-09-17.
...
created_at   timestamptz not null default now()
```
Added in `0012_three_tier_billing.sql` (D25). RLS on, no policies — service-role only. Single redemption enforced by conditional update (`... set redeemed_by = $1 where code = $2 and redeemed_by is null returning id`) — race-safe. Redemption creates a Stripe subscription on the `elevated_monthly` price with a 365-day trial and `trial_settings.end_behavior.missing_payment_method = 'cancel'`.
...
created_at      timestamptz not null default now()
```
Display copy says "Guide"; code identifiers stay `sage*` (frozen).
...
```
Added in `0013_pages_system.sql`. Admin-authored editorial guides (`/guides/[slug]`) and Sage pages (`/sages/[slug]`). RLS on, no policies (service-role only). Only `status='published'` pages render publicly, appear in the sitemap, or emit JSON-LD. Admin editing gated on `user.id === ADMIN_USER_ID` (interim, TD11).
...
updated_at  timestamptz not null default now()
```
Index `page_blocks_page_id_sort (page_id, sort_order)`. `content` shapes by `block_type`: `heading { text, level: 'h2'|'h3' }`, `paragraph { text }`, `image { url, alt, width: 'full'|'inset' }`, `image_text { url, alt, text, image_side: 'left'|'right' }`.
